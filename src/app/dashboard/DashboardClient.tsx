...
function DashboardInner() {
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-6 lg:py-8 max-w-6xl">
      {/* Follow-up banners */}
      {overdueFollowUps.length > 0 && !dismissedOverdue && (
        <div className="mb-4 p-4 rounded-xl bg-red-50 border border-red-100 flex items-start gap-3">
          ...
          <button onClick={() => setDismissedOverdue(true)} className="text-red-300 hover:text-red-500 flex-shrink-0 transition-colors">
      ...
      {upcomingFollowUps.length > 0 && !dismissedUpcoming && (
        <div className="mb-4 p-4 rounded-xl bg-amber-50 border border-amber-100 flex items-start gap-3">
          ...
          <button onClick={() => setDismissedUpcoming(true)} className="text-amber-300 hover:text-amber-500 flex-shrink-0 transition-colors">
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
            </svg>
          </button>
...
export default function DashboardClient() {
  return (
    <ToastProvider>
      <Suspense fallback={<div className="px-4 py-8"><div className="h-8 w-48 bg-slate-100 rounded-lg animate-pulse mb-4" /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[1,2,3].map(i => <div key={i} className="h-48 bg-slate-100 rounded-2xl animate-pulse" />)}</div></div>}>
        <DashboardInner />
      </Suspense>
    </ToastProvider>
  );
